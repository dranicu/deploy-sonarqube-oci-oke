...
output "sonarqube_url" {
  value = (var.deploy_nginx && var.deploy_sonarqube && length(coalesce(data.oci_load_balancer_load_balancers.lbs.load_balancers, [])) > 0 ?
    "https://sonarqube.${data.oci_load_balancer_load_balancers.lbs.load_balancers[0].ip_addresses[0]}.nip.io" :
    ""
  )
}
